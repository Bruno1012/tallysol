// One-off dev script: creates a devnet SPL token mint standing in for a
// real-world currency that has no official devnet test token (USDT, SKR, ...).
// Usage: node scripts/create-test-token-mint.mjs <SYMBOL> <mintAmount>
// Example: node scripts/create-test-token-mint.mjs USDT 10000
...
const MINT_AMOUNT_UI = Number(process.argv[3] ?? 1000)
...
async function main() {
  ...
  console.log(`New ${SYMBOL} (test) mint:`, mint.address)
  ...
  const mintToRecipient = getMintToInstruction({
    ...
    mintAuthority: payer,
    amount: BigInt(MINT_AMOUNT_UI) * 10n ** BigInt(DECIMALS),
  })
  ...
  console.log('')
  console.log(`DONE. ${SYMBOL} mint address to put in constants/currencies.ts:`)
